...
local function save_usage_data()
    local encodedTimers = vim.fn.json_encode(usage_data)
...
function M.show_aggregation(key, _start_date, _end_date)
    local function show_agg_helper(_key)
        ...
        local data = agg.aggregate(usage_data, _key, start_date_timestamp, end_date_timestamp)
    ...
    local valid_keys = { "filetype", "project", "filepath" }
    if key and not vim.tbl_contains(valid_keys, key) then
        vim.notify("UsageTracker: Aggregation key " .. key .. " is not valid", vim.log.levels.WARN)
        key = ""
    end
    ...
    if not key or key == "" then
        vim.ui.select({ "filetype", "project", "filepath" }, {
            ...
            if choice then
                show_agg_helper(choice)
            else
                vim.notify("UsageTracker: No aggregation key was chosen. Do nothing.", vim.log.levels.WARN)
            end
        end)
    else
        show_agg_helper(key)
...
local function cleanup_visit_log(filepath, days)
    local visit_log = usage_data.data[filepath].visit_log
    local now = os.time()
...
function M.setup(opts)
    ...
    if M.config.telemetry_endpoint and M.config.telemetry_endpoint ~= "" then
        ...
        if res.status ~= 200 then
            vim.api.nvim_echo({
                ...
                {
                    "Telemetry service is configured, but server is not responding: "
    ...
    vim.api.nvim_create_user_command("UsageTrackerHTML", function()
        require("usage-tracker.html").create_html_stats()
    end, { nargs = "?" })

    -- Clean up the visit log
    --TODO: Upon cleanup, the old logs should be preserverd with a suffix
    local now = os.time()
    if now - usage_data.last_cleanup > (M.config.cleanup_freq_days * 24 * 60 * 60) then
        for filepath, _ in pairs(usage_data.data) do
            cleanup_visit_log(filepath, M.config.keep_eventlog_days)
        end
